...
try {
  await go();
  await check('homepage is the menu: no global product navigation', async () => {
  ...
  await browser.close();
  await mkdir('docs/design/redesign-v2/evidence', { recursive: true });
  await writeFile('docs/design/redesign-v2/evidence/foundation.json', JSON.stringify({ base, results, errors }, null, 2) + '\n');
}
