...
const out = path.resolve('.impeccable/review/redesign-v2');
await mkdir(out, { recursive: true });
const browser = await chromium.launch();
const routes = [
  ['home','/'], ['muse','/products/muse'], ['layouts','/products/layout-compositions'],
  ['muse-detail','/products/muse/multi-action-button'], ['muse-image','/products/muse/file-management-dashboard'], ['layout-detail','/products/layout-compositions/001'], ['404','/does-not-exist'],
];
const evidence = [];
try {
for (const [device,width,height] of [['desktop',1440,900],['tablet',1024,768],['mobile',390,844]]) {
 ...
 for (const [name,route] of routes) {
  ...
  if(name==='muse-detail') await page.waitForFunction(()=>{const v=document.querySelector('video');return v && v.readyState>=4;},null,{timeout:18000});
  if (name === 'muse-image') await page.getByRole('region', {name:'作品媒体'}).locator('img').first().evaluate(img => img.complete && img.naturalWidth > 0 ? undefined : new Promise((resolve,reject) => { img.addEventListener('load',resolve,{once:true}); img.addEventListener('error',reject,{once:true}); setTimeout(() => reject(new Error('Media capture did not settle')),16000); }));
  if(name==='muse-detail') await page.waitForTimeout(2500);
  const file = `${name}-${device}.png`;
  await page.screenshot({path:path.join(out,file),fullPage:!['muse','home','layouts','muse-detail'].includes(name)});
  ...
  if(name==='muse-detail') {
    ...
    const infoFile=`muse-detail-info-${device}.png`; await page.screenshot({path:path.join(out,infoFile)});
    evidence.push({name:'muse-detail-info',device,route,file:infoFile});
  }
...
for (const [name,route] of routes.slice(0,6)) {
 ...
 if(name==='muse-image') await page.waitForFunction(()=>{const img=document.querySelector('[aria-label="作品媒体"] img'); return img?.complete && img.naturalWidth>0;});
 if(name==='muse-detail') await page.waitForFunction(()=>{const v=document.querySelector('video');return v && v.readyState>=4;},null,{timeout:18000});
