...
首页页头名称为「作品时间轴」，正文不重复显示同名标题，年份在时间轴上方；内页顶部用当前项目名（布局参考／灵感集／个人网站）作为标题链接，点击返回首页，左侧常驻返回箭头，悬停时箭头轻微向左上移动并提供「返回首页」提示，右侧保留主题切换；列表与项目介绍正文不重复显示同名标题，具体作品详情保留作品名称。日期位于1px轨道上方，产品块位于下方。产品按 `date` 升序、同日维持注册顺序；列宽420/380/340/290px适配现有断点。末端虚线对应「未完待续」。每件作品一个原生链接，整卡可点击，仅保留标题右上角的进入箭头，不再显示底部「浏览作品」及重复箭头；内页标题左上返回箭头与其对应。布局预览展示三张真实图鉴，灵感使用真实动态媒体。时间轴已支持独立站点外链入口及收录日期标记。第三个「个人网站」预览迁移原网站 AboutPrint 的个人经历小票打印动效，保留 2.4 秒逐段进纸节奏，打印完成后停留 1 秒，再淡入原网站首页截图，展示约 5.6 秒后返回打印，总周期 10 秒；截图加载成功才启用切换，失败则保留完整小票；使用 CSS 实现，不引入动画库，离屏／后台暂停，减少动态效果时显示完整小票。点击整个作品仍进入详情。
...
首页第三个作品进入站内介绍页。自然文档流中依次呈现返回时间轴、项目标题与简短介绍、打开原网站的主按钮、36 秒真实页面宣传片。沿用中性色与 1200px 内容宽度；宣传片可视时默认静音循环播放，成片无音轨；不显示播放器控制栏或遮罩，点击画面或按空格／Enter 切换播放与暂停，键盘聚焦保留可见焦点，离屏、后台与减少动态效果时暂停；不再展示功能截图区或底部「继续探索」模块，原网站入口仅保留顶部主按钮。介绍仅陈述原站实际功能，不新增营销数据。原网站在新标签页打开，保留介绍页阅读位置。
...
2026-09-09 标题与列表布局调整：项目首页的唯一 h1 位于顶部返回首页链接中，正文删除重复标题节点；两类列表使用分类与搜索同排的工具栏，窄屏自动换行，工具栏下直接展示结果与网格。作品详情保留自己的作品 h1。

2026-09-09 主题按钮参考个人网站：44px 圆形点击区、14.4px 月亮／太阳图标；浅色默认透明，深色使用次级表面与细边框，悬停提供主题对应底色，按下缩至 .97。图标轻微旋转并交叉淡入，键盘与减少动态效果即时切换；保留系统主题、用户记忆及跨标签同步。该专用按钮采用胶囊圆角，其余普通控件仍为 4px。
