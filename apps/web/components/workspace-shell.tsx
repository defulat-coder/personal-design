...
import { createContext, useContext, useEffect, useLayoutEffect, useRef, useState, type ComponentProps, type Dispatch, type SetStateAction } from 'react';
...
const WorkspaceNavigationContext = createContext<((href: string, scroll?: boolean) => boolean | void) | null>(null);

/** Keep Next's native link behavior (prefetch, modifier clicks, drag cancellation). */
export function WorkspaceLink({ href, ...props }: Omit<ComponentProps<typeof Link>, 'href' | 'onNavigate'> & { href: string }) {
...
export function WorkspaceShell({ children }: { children: React.ReactNode }) {
  ...
  const navigation = useRef<{ destination: string; productHref: string; floating: HTMLElement | null; target: HTMLElement | null; animations: Animation[]; exit: ReturnType<typeof playExit> | null; cleanup: () => void } | null>(null);
  ...
  function navigate(href: string, scroll?: boolean) {
    ...
    if (destination === pathname || !productHref || !(isHome || returning)
      || !products.some(product => product.href === productHref) || instantMotion()) return;

    const surface=shell.current!;
    surface.dataset.routeMotion='true';

    const source = shell.current?.querySelector<HTMLElement>(isHome
      ? `a[href="${productHref}"] h2` : '[data-workspace-title]');
    ...
    let cleaned = false;
    const pending = {
      destination, productHref, floating: floating ?? null, target: null as HTMLElement | null,
    ...
    if(returning) {
      ...
      pending.exit=playExit(shell.current?.querySelector('#workspace-content') ?? null,()=>router.push(href,{scroll}),undefined,{hold:true});
      return true;
    }
  }
  ...
  useLayoutEffect(() => {
    ...
    if (!pending) return;
    if (pathname !== pending.destination || instantMotion()) { pending.cleanup(); return; }
    pending.exit?.cancel();
    const target = shell.current?.querySelector<HTMLElement>(pathname === '/'
      ? `a[href="${pending.productHref}"] h2` : '[data-workspace-title]');
    ...
    const contentAnimation=shell.current?.querySelector('#workspace-content')?.animate([
      { opacity: .15, transform: `translateX(${distance}px) scale(.985)` },
      { opacity: 1, transform: 'none' },
    ], { duration: 360, easing: 'cubic-bezier(.23,1,.32,1)' });
    if(contentAnimation)pending.animations.push(contentAnimation);
    if (!target || !floating || !from || !to) { pending.cleanup(); return; }
    if (!from.width || !from.height || to.right < 0 || to.left > innerWidth) { pending.cleanup(); return; }
    ...
    const timeout = setTimeout(() => { animation.cancel(); pending.cleanup(); }, 500);
    void animation.finished.then(pending.cleanup, pending.cleanup).finally(() => clearTimeout(timeout));
  }, [pathname]);

  useEffect(() => () => navigation.current?.cleanup(), []);
  useEffect(() => observeMotionPolicy(() => {
  ...
  useEffect(() => {
    const keyboard = () => { if(document.documentElement.dataset.input !== 'keyboard')document.documentElement.dataset.input = 'keyboard'; };
    const pointer = () => { if(document.documentElement.dataset.input !== 'pointer')document.documentElement.dataset.input = 'pointer'; };
