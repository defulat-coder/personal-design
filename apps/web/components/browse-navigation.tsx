...
function Navigation({ appearance = 'button', returnLabel, fallbackHref, currentHref, entries, listPath, fromList = false }: Props & { fromList?:boolean }) {
  ...
  return <>
    <nav className={`${styles.navigation} ${textNavigation ? styles.textNavigation : ''}`} aria-label="作品导航">
      <Link href={fallbackHref} scroll={false} data-direction="previous" className={textNavigation ? `${styles.textControl} ${styles.returnLink}` : buttonClassName({ variant:'ghost' })}><ArrowLeft size={16} aria-hidden />{returnLabel}</Link>
