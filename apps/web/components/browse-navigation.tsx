...
function Navigation({ appearance = 'button', returnLabel, fallbackHref, currentHref, entries, listPath, fromList = false }: Props & { fromList?:boolean }) {
  ...
  return <>
    <nav className={`${styles.navigation} ${textNavigation ? styles.textNavigation : ''}`} aria-label="作品导航">
      <WorkspaceLink href={fallbackHref} scroll={false} data-direction="previous" className={textNavigation ? `${styles.textControl} ${styles.returnLink}` : buttonClassName({ variant:'ghost' })}><ArrowLeft size={16} aria-hidden />{returnLabel}</WorkspaceLink>
      <div className={styles.adjacent}>
        {prev ? <Link href={href(prev)} data-direction="previous" title={prev.title} aria-label={`上一件：${prev.title}`} className={controlClass}><PreviousIcon size={24} strokeWidth={1.5} aria-hidden /><span>上一件</span></Link> : <button type="button" className={controlClass} disabled aria-label="已是第一件"><PreviousIcon size={24} strokeWidth={1.5} aria-hidden /><span>上一件</span></button>}
        {next ? <Link href={href(next)} data-direction="next" title={next.title} aria-label={`下一件：${next.title}`} className={controlClass}><span>下一件</span><NextIcon size={24} strokeWidth={1.5} aria-hidden /></Link> : <button type="button" className={controlClass} disabled aria-label="已是最后一件"><span>下一件</span><NextIcon size={24} strokeWidth={1.5} aria-hidden /></button>}
      </div>
    </nav>
