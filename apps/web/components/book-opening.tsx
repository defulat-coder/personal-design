...
export function BookOpening({book,onDone}:{book:OpeningBook;onDone:()=>void}) {
  ...
  useEffect(()=>{
    ...
    const width=book.extracted ? book.rect.width : Math.min(300,window.innerWidth*.38);
    const height=book.extracted ? book.rect.height : Math.min(400,window.innerHeight*.56);
