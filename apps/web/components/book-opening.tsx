...
export function BookOpening({book,onDone}:{book:OpeningBook;onDone:()=>void}) {
  ...
  useEffect(()=>{
    ...
    const width=Math.min(300,window.innerWidth*.38);
    const height=Math.min(400,window.innerHeight*.56);
    ...
    const run=async()=>{
      ...
      if(stopped)return;
      await Promise.all([
        play(actor,[{transform:`translate(${book.rect.left}px,${book.rect.top-38}px)`,width:`${book.rect.width}px`,height:`${book.rect.height}px`},{transform:center,width:`${width}px`,height:`${height}px`}],620),
        play(front,[{transform:'rotateY(88deg)'},{transform:'rotateY(0deg)'}],620),
        play(side,[{opacity:1,transform:'rotateY(0deg)'},{opacity:0,transform:'rotateY(-90deg)',offset:.65},{opacity:0,transform:'rotateY(-90deg)'}],620),
      ]);
      if(stopped)return;
  ...
  return <div className={styles.overlay} aria-hidden="true">
    <div ref={root} className={styles.book} style={{'--cover':book.color,'--cover-ink':book.ink,width:book.rect.width,height:book.rect.height,transform:`translate(${book.rect.left}px,${book.rect.top}px)`} as CSSProperties}>
      <div ref={spine} className={styles.spine} style={{width:book.rect.width}}>{book.title}</div>
      <div ref={cover} className={styles.cover}><span>{book.title}</span><small>排版构图图鉴</small></div>
    </div>
  </div>;
}
