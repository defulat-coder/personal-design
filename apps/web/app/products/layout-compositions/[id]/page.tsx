...
import { DetailKeyboardNav, DetailMainImage, LayoutReturnLink } from '@/components/detail-tools';
...
export default async function LayoutDetailPage({ params }: PageProps) {
  ...
  return <main className={styles.page}>
    <div className={styles.navigation}>
      <LayoutReturnLink />
      <nav aria-label="同主题翻页" className={styles.pagination}>
        {prev ? <Link href={detailHref(prev)} className={buttonClassName({ variant: 'ghost' })} aria-label={`上一张：${prev.name}`}><ArrowLeft aria-hidden size={16} />上一张</Link> : <span className={styles.boundary}>本主题第一张</span>}
        <span className={styles.position}>{index + 1} / {group.length}</span>
        {next ? <Link href={detailHref(next)} className={buttonClassName({ variant: 'ghost' })} aria-label={`下一张：${next.name}`}>下一张<ArrowRight aria-hidden size={16} /></Link> : <span className={styles.boundary}>本主题最后一张</span>}
      </nav>
    </div>
    ...
    {src ? <section className={styles.artwork} aria-label="图鉴图片">
      <div className={styles.media}>
        <DetailMainImage key={item.id} src={src} thumb={thumbnailUrl(item)} alt={item.name} serial={item.id} siblings={siblings} index={withImage.findIndex((entry) => entry.id === item.id)} />
      </div>
      <div className={styles.resource}>
        <p>点击图片放大查看 <span className={styles.serial}>{item.width} × {item.height}</span></p>
        <a href={src} {...(src.startsWith('/') ? { download: `${item.id}-${item.name}.webp` } : { target: '_blank', rel: 'noopener noreferrer' })} className={buttonClassName()}>
          {src.startsWith('/') ? <Download aria-hidden size={16} /> : <ExternalLink aria-hidden size={16} />}{src.startsWith('/') ? '下载高清图' : '打开高清原图'}
        </a>
      </div>
    ...
    <DetailKeyboardNav prevHref={prev ? detailHref(prev) : undefined} nextHref={next ? detailHref(next) : undefined} />
  </main>;
}
