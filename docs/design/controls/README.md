...
两集合支持即时搜索输入、搜索快捷键及搜索清除／提交按钮。分类、主题筛选与浏览恢复保留。
...
| 普通标签分类 | CategoryTabs | 44px目标、选中1px下划线、aria-pressed；计数不分散在分类标签 |
...
| 视频 | MotionVideo / SiteShowcaseMedia | 灵感专注播放器保留原生控制；个人网站宣传片采用点击／键盘切换的已确认例外 |
...
- 共享按钮焦点环2px、外偏3px；链接与分类按局部间距采用4–8px；forced-colors 恢复明确轮廓。
...
灵感集恢复CategoryTabs中文分类和PlateWall原生作品链接；详情复用BrowseNavigation返回与相邻导航、原生视频控制及图片灯箱。不再使用放映台或TypeDial。
