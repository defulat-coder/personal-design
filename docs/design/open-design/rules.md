...
- 加载不能无限持续；筛选空状态保留分类／主题上下文，提供可行的清除或调整途径。
...
| 分类与主题 | 共享即时搜索；中文分类以下划线选中，计数集中结果区；灵感按标题、作者、标签检索，布局按名称、编号、主题检索 |
| 内容 | 媒体8px圆角，开放阅读流；普通页面不增加装饰投影和层层面板 |
| 局部材质 | 八本分类书、米白纸面、装订阴影与首页书籍预览允许实体书造型，不扩展到全站；灵感恢复普通网格 |
| 动效 | 通用进入200ms / 退出140ms；按钮缩放100ms；首页导航360ms；书籍使用专属序列，页脚上一页／下一页保留翻页反馈。键盘、减少动态效果与中断路径按 DESIGN.md 验收 |
| 内容浏览 | 灵感网格、纵向自动追加、原生链接进详情；图片与视频共存。布局为书架和画册，图片点击放大；无JSON／同步信息 |
| 导航 | 首页选择产品；页头按当前层级返回；灵感详情沿分类与关键词结果连续浏览，返回保留位置；旧布局图鉴地址进入对应画册放大 |
