...
test('读取分段 RSC 中的列表，保留字符串中的括号和转义字符', () => {
  ...
  const html = [payload.slice(0, 40), payload.slice(40)]
    .map((part) => `<script>self.__next_f.push([1,${JSON.stringify(part)}])</script>`).join('');
...
test('详情跳过同 ID 的列表条目，读取带出处的完整对象', () => {
  ...
  assert.deepEqual(extractDetailPost(`<script>self.__next_f.push([1,${JSON.stringify(payload)}])</script>`, 'new'), detail);
});

test('需要翻页时持续读取到旧作品，完整合并新增', async () => {
  const result = await collectFeed(['Web'], new Set(['old']), async (_, cursor) => cursor
...
test('各分类分别追到原有作品，合并新增时不提前停止其他分类', async () => {
  const result = await collectFeed(['Web', 'Motion'], new Set(['old']), async (category) => ({
    items: [{ id: 'shared' }, { id: category }, { id: 'old' }], nextCursor: 'more',
  }));
  assert.deepEqual(result.map((item) => item.id), ['shared', 'Web', 'Motion']);
});

test('首屏没有覆盖增量时必须翻页，接口失败不能返回不完整结果', async () => {
  await assert.rejects(collectFeed(['Web'], new Set(['old']), async (_, cursor) => {
    if (cursor) throw new Error('HTTP 429 checkpoint');
    return { items: [{ id: 'new' }], nextCursor: 'more' };
  }), /429/);
  await assert.rejects(collectFeed(['Web'], new Set(), async () => ({
    items: [{ id: 'new' }], nextCursor: 'more',
  }), { maxPages: 1 }), /未覆盖/);
});
